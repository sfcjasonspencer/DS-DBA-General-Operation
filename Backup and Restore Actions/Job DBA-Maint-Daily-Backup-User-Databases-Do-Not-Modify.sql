...
/****** Object:  Job [DBA-Maint-Daily-Backup-User-Databases-Do-Not-Modify]    Script Date: 2/18/2019 12:47:10 PM ******/
EXEC msdb.dbo.sp_delete_job @job_name=N'DBA-Maint-Daily-Backup-User-Databases-Do-Not-Modify', @delete_unused_schedule=1
GO
...
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 2/18/2019 12:47:10 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA-Maint-Daily-Backup-User-Databases-Do-Not-Modify', 
		@enabled=0, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Step One - Daily Backup User Databases]    Script Date: 2/18/2019 12:47:10 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Step One - Daily Backup User Databases', 
		@step_id=1, 
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Step Two - Daily TRN Backup User Databases]    Script Date: 2/18/2019 12:47:10 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Step Two - Daily TRN Backup User Databases', 
		@step_id=2, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_success_step_id=0, 
...
-- specify database backup directory
SET @path = ''\\HXCNAS2\SQL-Server-Backups\'' + @ServerName + ''\'' + @InstanceName + ''\Daily\'' + @dayofweekdate  + ''\Transactional\User\''
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Daily User Database Backup Schedule', 
		...
		@schedule_uid=N'ee00a40b-4751-41c9-94e2-cc66feb0d1e9'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
